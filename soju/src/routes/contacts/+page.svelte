<script lang="ts">
	...
	interface Props {
		data: { contacts: Contact[]; title: string };
	}

	let { data }: Props = $props();

	let contacts: Contact[] | undefined = $state();
	$effect(() => {
		contacts = data.contacts;
	}); // Contacts for reactive state
					...
					<header class="card-header flex gap-3">
						<div class="flex flex-col">
							...
							<p class="flex items-center gap-1">
								<Icon icon="akar-icons:phone" />{contact.mobile}
							</p>
						</div>
					</header>
					<section class="flex justify-center items-center card-footer gap-1">
						<button
							onclick={() => goto(`/contacts/${contact._id}`)}
